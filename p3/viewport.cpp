...
Viewport::Viewport() :
		gdata(Gamedata::getInstance()), position(0, 0), viewWidth(
				gdata.getXmlInt("view/width")), viewHeight(
				gdata.getXmlInt("view/height")), worldWidth(
				gdata.getXmlInt("world/width")), worldHeight(
				gdata.getXmlInt("world/height")), objWidth(0), objHeight(0), objectToTrack(
				NULL)
{}
...
void Viewport::draw() const {
	IOManager::getInstance().printMessageCenteredAt(
			"Tracking " + objectToTrack->getName(), 30);
}
