...
BulletPool::BulletPool() : bullets(), freeList(){}
...
void BulletPool::drawMsg(){
	...
	IOManager::getInstance().printMessageAt("freelist size = " + s.str(), 150, 20);
	...
	for (std::list<Bullet*>::iterator iter = this->bullets.begin();
		...
		IOManager::getInstance().printMessageAt(ss.str(), 150, i * 15 + 20);
		}
