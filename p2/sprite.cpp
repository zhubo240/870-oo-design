...
Sprite::Sprite(const std::string& name, const Frame* const frame) :
		Drawable(name,
				...
				Vector2f(Gamedata::getInstance()->getXmlInt(name + "/speedX"),
						Gamedata::getInstance()->getXmlInt(name + "/speedY"))), frame(
				frame), frameWidth(frame->getWidth()), frameHeight(
...
Sprite::Sprite(const Sprite& s) :
		Drawable(s), frame(s.frame),  // shallow copy; Manager may reuse it
		frameWidth(s.getFrame()->getWidth()), frameHeight(
