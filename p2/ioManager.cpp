...
IOManager::IOManager() :
		gdata(Gamedata::getInstance()), viewWidth(
				gdata->getXmlInt("view/width")), viewHeight(
				gdata->getXmlInt("view/height")), MAX_STRING_SIZE(
				gdata->getXmlInt("maxStringSize")),
		// The 3rd and 4th parameters are just as important as the first 2!
		screen(SDL_SetVideoMode(viewWidth, viewHeight, 32, SDL_DOUBLEBUF)), font(
				NULL), color(), title(gdata->getXmlStr("screenTitle")), inputString() {
	if (screen == NULL) {
	...
	font = TTF_OpenFont(gdata->getXmlStr("font/file").c_str(),
...
SDL_Surface* IOManager::loadAndSet(const string& filename,
...
void IOManager::printMessageAt(const string& msg, Sint16 x, Sint16 y) const {
	...
	if (stext) {
		...
		throw string("Couldn't allocate text sureface in printMessageAt");
	}
}
...
void IOManager::printMessageCenteredAt(const string& msg, Sint16 y) const {
	...
	if (stext) {
		...
		throw string(
				"Couldn't allocate text sureface in printMessageCenteredAt");
	}
}

void IOManager::printStringAfterMessage(const string& msg, Sint16 x,
		Sint16 y) const {
...
template<typename T>
void IOManager::printMessageValueAt(const string& msg, T value, Sint16 x,
		Sint16 y) const {
	...
	if (stext) {
		...
		throw string("Couldn't allocate text sureface in printMessageValueAt");
	}
}

template void IOManager::printMessageValueAt(const string& msg, unsigned long,
		Sint16, Sint16) const;

template void IOManager::printMessageValueAt(const string& msg, float, Sint16,
		Sint16) const;

template void IOManager::printMessageValueAt(const string& msg, unsigned,
		Sint16, Sint16) const;

template void IOManager::printMessageValueAt(const string& msg, int, Sint16,
		Sint16) const;
